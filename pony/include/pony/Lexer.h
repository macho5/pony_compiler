...
public:
  ...
  /// A queue to store tokens
  std::queue<std::string> getQueue() {return tokenQueue;}   // the first part

  /// Whether there is invalid identifier
  bool getValid() {return isValid;}   // the first part
...
private:
  ...
  int getNextChar() {
    ...
    if(nextChar == '\n'){
      ...
      // make the curLineBuffer not empty (except EOF)
  ...
  Token getTok() {
    ...
    if(isalpha(lastChar) || lastChar == '_'){
      bool valid = 1;
      bool number_before = 0;
      std::string buffer;
      while(isalnum(lastChar) || lastChar == '_'){
        // confirm valid
        if(number_before && (isalpha(lastChar) || lastChar == '_')){
          valid = 0;
          isValid = 0;
          std::cerr << "Token error (" << curLineNum << ',' << curCol << "): digits should be at the end of an identifier!" << std::endl;
          break;
        }
        if(isdigit(lastChar)){
          number_before = 1;
    ...
    if (isdigit(lastChar) || lastChar == '.') {
      ...
      std::string numStr;
      if(numStr == "" && lastChar == '.'){
        std::cerr << "Token error (" << curLineNum << ',' << curCol << "): a digit should appear before a dot!" << std::endl;
        isValid = 0;
      }
      while(isdigit(lastChar) || lastChar == '.'){
        if(lastChar == '.' && dot_before){
          std::cerr << "Token error (" << curLineNum << ',' << curCol << "): a number should at most contain one dot!" << std::endl;
          isValid = 0;
        }
        if(lastChar == '.')
          dot_before = 1;
    ...
    if(thisChar == ';' || thisChar == '(' || thisChar == '[' || thisChar == '{' 
      ...
      tokenQueue.push(std::string(1, thisChar));    // the first part
    return thisChar;
  }
  ...
  /// A queue to store tokens
  std::queue<std::string> tokenQueue;   // the first part

  /// Whether there is invalid identifier
  bool isValid = 1;   // the first part
};
