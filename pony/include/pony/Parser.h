...
private:
  ...
  std::unique_ptr<VarDeclExprAST> parseDeclaration() {
    ...
    if(lexer.getCurToken() != tok_var)
      return parseError<VarDeclExprAST>("var", "when declaring a variable");
    lexer.getNextToken();
    ...
    if(lexer.getCurToken() != tok_identifier)
      return parseError<VarDeclExprAST>("an identifier", "after \"var\" declaration");
  ...
  std::unique_ptr<VarType> parseType() {
    ...
    if(lexer.getCurToken() == '<'){
      ...
      if (lexer.getCurToken() != ']')
        return parseError<VarType>("]", "to end row");
      lexer.getNextToken(); // eat ]

      if (lexer.getCurToken() != '[')
        return parseError<VarType>("[", "to start column");
      lexer.getNextToken(); // eat [
  ...
  std::unique_ptr<ExprAST> parseIdentifierExpr() {
    std::string id = static_cast<std::string>(lexer.getId());   // get current identifier
    auto loc = lexer.getLastLocation();
    lexer.getNextToken();   // eat identifier
    if(lexer.getCurToken() != '('){   // just a variable
      return std::make_unique<VariableExprAST>(std::move(loc), id);
    }

    // a function call
    lexer.getNextToken();   // eat (
    std::vector<std::unique_ptr<ExprAST>> args;   // store arguments
    ...
    while(lexer.getCurToken() != ')'){
      ...
      if(lexer.getCurToken() != ',')
        return parseError<ExprAST>("an end identifier or ,", "between arguments");
      lexer.getNextToken();
    }
    lexer.getNextToken();   // eat )
    
    if(id == "print"){
      if(args.size() == 1)  // only one argument, meeting the requirement
        return std::make_unique<PrintExprAST>(loc, std::move(args[0]));
      else 
        return parseError<ExprAST>("<single arg>", "for the builtin print() function");
    }

    return std::make_unique<CallExprAST>(loc, std::move(id), std::move(args));
  }
  ...
  std::unique_ptr<ExprAST> parseBinOpRHS(int exprPrec,
    ...
    while(1){
      int thisPrec = getTokPrecedence();
      if(thisPrec < exprPrec)
        return lhs;
      pony::Token op = lexer.getCurToken();
      ...
      if(!rhs)
        return parseError<ExprAST>("an expression", "after a binary operator");

      int nextPrec = getTokPrecedence();
      if(thisPrec < nextPrec){
        rhs = parseBinOpRHS(thisPrec, std::move(rhs));
        if(!rhs)
          return nullptr;
      }
      lhs = std::make_unique<BinaryExprAST>(std::move(loc), static_cast<char>(op), std::move(lhs), std::move(rhs));    // lhs merged with rhs
    }
